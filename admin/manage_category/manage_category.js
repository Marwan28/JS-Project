        ...
        if (xhrProd.readyState === 4 && xhrProd.status === 200) {
          ...
          for (var i = 0; i < categories.length; i++) {
            ...
            for (var j = 0; j < products.length; j++) {
              if (products[j].category_id === c.id) {
                count++;
              }
            }

            var imgTag = c.categoryImg
              ? '<img src="' +
                c.categoryImg +
...
document.querySelector(".btn-add").onclick = function () {
  ...
  catInput.style.borderColor = "#ddd";
  imgInput.style.borderColor = "#ddd";
};
...
document.getElementById("btnSave").onclick = function () {
  ...
  if (nameVal === "") {
    ...
    catInput.style.borderColor = "#dc3545";
    isValid = false;
  } else {
    var isDuplicate = false;
    for (var i = 0; i < existingCategories.length; i++) {
      if (existingCategories[i].name.toLowerCase() === nameVal.toLowerCase()) {
        isDuplicate = true;
        break;
      }
    }
    if (isDuplicate) {
      nameError.innerHTML = "This category already exists!";
      nameError.style.display = "block";
      catInput.style.borderColor = "#dc3545";
      isValid = false;
    } else {
  ...
  if (imgVal === "") {
    ...
    imgInput.style.borderColor = "#dc3545";
    isValid = false;
  } else {
    imgError.style.display = "none";
    imgInput.style.borderColor = "#ddd";
...
function saveCategory(name, imgUrl) {
  ...
  xhrAdd.onreadystatechange = function () {
    if (
      xhrAdd.readyState === 4 &&
      (xhrAdd.status === 201 || xhrAdd.status === 200)
    ) {
 ...
 window.editCategory = function(id) {
  ...
  var target = null;
  for (var i = 0; i < existingCategories.length; i++) {
    if (existingCategories[i].id === id) {
      target = existingCategories[i];
      break;
    }
  }
  if (target) {
    ...
    editImgError.style.display = "none";
    editCatInput.style.borderColor = "#ddd";
    editImgInput.style.borderColor = "#ddd";
  }
}
...
btnUpdateSave.onclick = function () {
  ...
  if (nameVal === "") {
    editNameError.innerHTML = "Name is required";
    editNameError.style.display = "block";
    editCatInput.style.borderColor = "#dc3545";
    isValid = false;
  } else {
    var isDuplicate = false;
    for (var i = 0; i < existingCategories.length; i++) {
      if (
        existingCategories[i].name.toLowerCase() === nameVal.toLowerCase() &&
        existingCategories[i].id !== currentEditingId
      ) {
        isDuplicate = true;
        break;
      }
    ...
    if (isDuplicate) {
      editNameError.innerHTML = "Name already taken by another category!";
      editNameError.style.display = "block";
      editCatInput.style.borderColor = "#dc3545";
      isValid = false;
    } else {
  ...
  if (imgVal === "") {
    editImgError.style.display = "block";
    editImgInput.style.borderColor = "#dc3545";
    isValid = false;
  } else {
    editImgError.style.display = "none";
    editImgInput.style.borderColor = "#ddd";
  ...
  if (isValid && currentEditingId !== null) {
    ...
    xhrUpdate.open(
      "PUT",
      supabaseUrl + "/rest/v1/category?id=eq." + currentEditingId,
      true,
    );
    ...
    xhrUpdate.onreadystatechange = function () {
      if (
        ...
        (xhrUpdate.status === 204 || xhrUpdate.status === 200)
      ) {
...
document.getElementById("btnConfirmDelete").onclick = function () {
  if (categoryIdToDelete !== null) {
    ...
    xhrDel.open(
      "DELETE",
      supabaseUrl + "/rest/v1/category?id=eq." + categoryIdToDelete,
      true,
    );
      ...
      if (xhrDel.readyState === 4) {
        if (xhrDel.status === 204 || xhrDel.status === 200) {
          ...
          if (errorResponse.code === "23503") {
            alert(
              "Cannot delete this category because it contains products. Please delete the products first.",
            );
            document.getElementById("deleteModal").style.display = "none";
          } else {
...
document.getElementById("btnEditClose").onclick = function () {
  editModal.style.display = "none";
};
